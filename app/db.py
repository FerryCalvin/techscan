...
if _explicit_url:
    ...
    if not db_pass:
        if os.environ.get("TECHSCAN_ALLOW_EMPTY_DB_PASSWORD", "0") == "1":
            ...
            raise RuntimeError(
                "TECHSCAN_DB_PASSWORD is not set. Define TECHSCAN_DB_URL or set TECHSCAN_DB_PASSWORD via environment."
            )
...
def start_pool_monitor(interval_s: float = 10.0):
    ...
    t = threading.Thread(target=_pool_monitor_loop, args=(interval,), daemon=True, name="db-pool-monitor")
...
def save_scan(result: dict, from_cache: bool, timeout_used: int):
    ...
    base_now = time.time()
    started_at = _coerce_epoch(
        result.get("started_at") or result.get("_started_at") or result.get("timestamp"), base_now
    )
    finished_at = _coerce_epoch(
        result.get("finished_at")
        or result.get("_finished_at")
    ...
    if payload_bytes is None:
        try:
            ...
            logging.getLogger("techscan.db").debug(
                "failed estimating payload size for domain=%s", result.get("domain"), exc_info=True
            )
            payload_bytes = None
    try:
        ...
        logging.getLogger("techscan.db").debug(
            "invalid payload size for domain=%s payload=%s", result.get("domain"), payload_bytes, exc_info=True
        )
        payload_bytes = None
    log = logging.getLogger("techscan.db")
    with get_conn() as conn:
        with conn.cursor() as cur:
            try:
                ...
                log.warning(
                    "insert_scans_failed domain=%s mode=%s err=%s",
                    result.get("domain"),
            ...
            for tech in technologies:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    log.warning(
                        "upsert_domain_tech_failed domain=%s tech=%s version=%s err=%s",
                        result["domain"],
                        name,
                        version,
                        up_ex,
        ...
        conn.commit()
    log.debug(
        "save_scan_ok domain=%s mode=%s tech_count=%s versions_with=%s cache=%s timeout_used=%s",
        result.get("domain"),
        result.get("scan_mode"),
        tech_count,
        versions_count,
...
def count_search_tech(
    tech: str | None = None, category: str | None = None, version: str | None = None, new24: bool = False
):
    tech_term, tech_param, tech_use_like = _prepare_tech_filter(tech)
    if _DB_DISABLED or _is_disabled_runtime():
        seen = set()
    ...
    # Cache key
    k = (
        tech_term.lower() if tech_term else None,
...
def history(domain: str, limit: int = 20, offset: int = 0):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """SELECT mode, started_at, finished_at, duration_ms, from_cache, adaptive_timeout, retries, timeout_used, payload_bytes
                           FROM scans WHERE domain=%s ORDER BY finished_at DESC LIMIT %s OFFSET %s""",
                (domain, limit, offset),
            )
            rows = cur.fetchall()
            out = []
...
def get_domain_techs(domain: str):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """SELECT tech_name, version, categories, first_seen, last_seen
                           FROM domain_techs WHERE domain=%s ORDER BY last_seen DESC""",
                (domain,),
            )
            rows = cur.fetchall()
            out = []
...
def get_latest_scan_raw(domain: str):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """SELECT raw_json, technologies_json, finished_at FROM scans WHERE domain=%s ORDER BY finished_at DESC LIMIT 1""",
                (domain,),
            )
            row = cur.fetchone()
            if not row:
...
def top_versions_for_tech(tech: str, limit: int = 10):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """SELECT version, COUNT(*) AS cnt FROM domain_techs WHERE LOWER(tech_name)=LOWER(%s) AND version IS NOT NULL GROUP BY version ORDER BY cnt DESC LIMIT %s""",
                (tech, limit),
            )
    ...
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                row = cur.fetchone()
                if row:
                    info["last_scan"] = {"domain": row[0], "finished_at": row[1].timestamp()}
        info["ok"] = True
    except Exception as e:
        info["error"] = str(e)
    return info
        ...
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                if dry_run:
                    ...
                    log.info(
                        "cleanup_old_scans DRY RUN: scans=%d techs=%d jobs=%d (retention=%d days)",
                        scans_count,
                        techs_count,
                        jobs_count,
                        retention_days,
                    ...
                    conn.commit()
                    log.info(
                        "cleanup_old_scans: deleted scans=%d techs=%d jobs=%d (retention=%d days)",
                        result["deleted_scans"],
                        result["deleted_domain_techs"],
                        result["deleted_jobs"],
                        retention_days,
                    )
...
def _cleanup_loop(interval_hours: float, retention_days: int):
    ...
    log.info("Cleanup scheduler started: interval=%s hours, retention=%s days", interval_hours, retention_days)
...
def start_cleanup_scheduler():
    ...
    if os.environ.get("TECHSCAN_CLEANUP_ENABLED", "0") != "1":
        logging.getLogger("techscan.db").debug("Cleanup scheduler disabled (set TECHSCAN_CLEANUP_ENABLED=1 to enable)")
        return False
    ...
    _CLEANUP_STOP.clear()
    t = threading.Thread(
        target=_cleanup_loop, args=(interval_hours, retention_days), daemon=True, name="db-cleanup-scheduler"
    )
    _CLEANUP_THREAD = t
    t.start()
    return True
