...
def _ensure_process() -> None:
    ...
    if not node_path:
        logging.getLogger('techscan.persist').warning('Node binary not found in PATH and TECHSCAN_NODE not set; persistent mode disabled')
        return
    ...
    for attempt in range(1, start_attempts + 1):
        try:
            ...
            time.sleep(0.15)
            if _proc and _proc.poll() is None:
                logging.getLogger('techscan.persist').info('persistent scanner daemon started pid=%s', getattr(_proc, 'pid', None))
                return
        except Exception as e:
            logging.getLogger('techscan.persist').warning('failed to start persistent scanner daemon (attempt %d): %s', attempt, e)
            try:
                if _proc:
                    _proc.kill()
            except Exception:
            ...
            _proc = None
            time.sleep(backoff)
    logging.getLogger('techscan.persist').error('exhausted persistent scanner start attempts; persistent mode unavailable')
