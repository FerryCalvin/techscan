...
from ..scanners.node import scan_domain, QUICK_DEFAULT_BUDGET_MS
...
def snapshot_cache(domains: list[str]) -> list[dict]:
    ...
    with _lock:
        ...
        # Construct keys
        [f"full:{d}" for d in domains] + [f"fast:{d}" for d in domains]
...
def bulk_quick_then_deep(domains: List[str], wappalyzer_path: str, concurrency: int = 4) -> List[Dict[str, Any]]:
    ...
    results = [None] * len(domains)
    with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
        future_to_idx = {executor.submit(deep_scan, d, wappalyzer_path): i for i, d in enumerate(domains)}
        for future in concurrent.futures.as_completed(future_to_idx):
            idx = future_to_idx[future]
            try:
                results[idx] = future.result()
...
def scan_unified(domain: str, wappalyzer_path: str, budget_ms: int = 25000) -> Dict[str, Any]:
    ...
    from ..utils.domain import validate_domain, extract_host
    from ..utils.tech_data import attach_raw_hint_meta, apply_hint_meta_detections, infer_tech_from_urls
    ...
    for cat, items in w_cats.items():
        bucket = merged["categories"].setdefault(str(cat), [])
        for item in items:
    ...
    if w_res.get("extras"):
        _merge_extras(extras, w_res["extras"])
    if w_res.get("raw", {}).get("extras"):
        _merge_extras(extras, w_res["raw"]["extras"])
    if w_res.get("data", {}).get("extras"):
        _merge_extras(extras, w_res["data"]["extras"])
