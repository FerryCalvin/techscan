...
from ..utils.tech_data import (
    load_categories,
    normalize_result,
    attach_raw_hint_meta as _attach_raw_hint_meta,
    apply_hint_meta_detections as _apply_hint_meta_detections,
)
...
def synthetic_header_detection(domain: str, timeout: int = 5) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if server:
            ...
            if name:
                techs.append(
                    {"name": name, "version": None, "categories": ["Web Servers"], "confidence": 100, "header": True}
                )
...
def scan_domain(
    domain: str, wappalyzer_path: str, timeout: int = 45, retries: int = 0, full: bool = False, url: str | None = None
) -> Dict[str, Any]:
    ...
    if not preflight(domain):
        ...
        if os.environ.get("TECHSCAN_TIERED", "0") == "1":
            try:
                ...
                hres = heuristic_fast.run_heuristic(
                    domain, budget_ms=int(os.environ.get("TECHSCAN_TIERED_BUDGET_MS", "1200")), allow_empty_early=True
                )
    ...
    if _check_quarantine(domain):
        ...
        if os.environ.get("TECHSCAN_TIERED", "0") == "1":
            try:
                ...
                hres = heuristic_fast.run_heuristic(
                    domain, budget_ms=int(os.environ.get("TECHSCAN_TIERED_BUDGET_MS", "1600")), allow_empty_early=True
                )
    ...
    if os.environ.get("TECHSCAN_ULTRA_QUICK", "0") == "1" and not full:
        try:
            ...
            uq["scan_mode"] = "fast"
            if (
                os.environ.get("TECHSCAN_SKIP_VERSION_AUDIT", "0") != "1"
                and os.environ.get("TECHSCAN_VERSION_AUDIT", "1") == "1"
            ):
                try:
            ...
            logging.getLogger("techscan.ultra").warning(
                "ultra quick path failed domain=%s err=%s (fall back)", domain, ue
            )
    ...
    local_scanner = (
        ...
        / "node_scanner"
        / ("scanner.js" if not persist else "server.js")
    )
    ...
    if use_py_wapp and not full:
        ...
        try:
            ...
            synthetic_allowed = (
                os.environ.get("TECHSCAN_SYNTHETIC_HEADERS", "1") == "1"
                and os.environ.get("TECHSCAN_DISABLE_SYNTHETIC", "0") != "1"
            )
                        ...
                        for tech in synth:
                            if tech["name"] not in existing_names:
                                ...
                                for cat in tech.get("categories", []):
                                    result["categories"].setdefault(cat, []).append(
                                        {"name": tech["name"], "version": tech.get("version")}
                                    )
                                added = True
                        if added:
                            logging.getLogger("techscan.synthetic").debug(
                                "added synthetic headers domain=%s items=%d", domain, len(synth)
                            )
                            with _stats_lock:
                                STATS["synthetic"]["headers"] += 1
                except Exception as se:
                    logging.getLogger("techscan.synthetic").debug(
                        "synthetic header detection failed domain=%s err=%s", domain, se
                    )
            synth_end = time.time()
            ...
            logger.info(
                "scan success domain=%s engine=%s duration=%.2fs", domain, result["engine"], (op_end - op_start)
            )
            ...
            logging.getLogger("techscan.scan_domain").warning(
                "py-local detector failed domain=%s err=%s (fallback to existing path)", domain, e
            )
    ...
    logger.debug(
        "effective_timeout domain=%s requested=%s effective=%s attempts=%d implicit_retry=%s full=%s",
        domain,
        timeout,
        eff_timeout,
        attempts,
        implicit_timeout_retry,
        full,
    )
    ...
    while attempt <= attempts:
        ...
        logger.debug(
            "scan start domain=%s attempt=%d/%d timeout=%s cmd=%s",
            domain,
            attempt,
            attempts,
            eff_timeout,
        ...
        try:
            ...
            env["TECHSCAN_NAV_TIMEOUT"] = (
                str(int(min(eff_timeout - 1, eff_timeout) * 1000)) if eff_timeout > 2 else str(int(eff_timeout * 1000))
            )
            if full:
                env["TECHSCAN_FULL"] = "1"
            ...
            result["phases"]["synthetic_ms"] = int((synth_end - synth_start) * 1000) if synthetic_allowed else 0
            logger.info(
                "scan success domain=%s engine=%s duration=%.2fs attempts=%d",
                domain,
                result["engine"],
                elapsed,
                attempt,
            ...
            if implicit_timeout_retry:
                ...
                if not disable_adaptive:
                    ...
                    if new_timeout > eff_timeout:
                        logger.warning(
                            "adaptive timeout bump domain=%s old=%ss new=%ss", domain, eff_timeout, new_timeout
                        )
