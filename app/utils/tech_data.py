...
def apply_hint_meta_detections(payload: Dict[str, Any]) -> None:
    ...
    for s in scripts:
        ...
        if 'jquery-migrate' in s_lower:
            ...
            techs.append({'name': 'jQuery Migrate', 'version': ver, 'confidence': 100, 'categories': ['JavaScript libraries']})
        ...
        if 'sitepress-multilingual-cms' in s_lower or 'sitepress.js' in s_lower:
            ...
            ver = extract_asset_version(s)
            # Check exist
            existing = next((t for t in techs if t['name'] == 'WPML' or t['name'] == 'WordPress Multilingual Plugin (WPML)'), None)
            if existing:
                if ver and not existing.get('version'):
                    existing['version'] = ver
            else:
                techs.append({'name': 'WordPress Multilingual Plugin (WPML)', 'version': ver, 'confidence': 100, 'categories': ['WordPress plugins']})
...
def infer_tech_from_urls(urls: List[str]) -> List[Dict[str, Any]]:
    ...
    if has_jsdelivr and 'jsDelivr' not in detected_names:
        hints.append({
            'name': 'jsDelivr', 
            'categories': INFERRED_CATEGORY_MAP.get('jsDelivr', []),
            'confidence': 100
        })
