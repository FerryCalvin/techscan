...
class TrainingDataGenerator:
    ...
    def fetch_html(self, domain: str, timeout: int = 10) -> Tuple[str, Dict[str, str]]:
        ...
        url = f"https://{domain}"
        try:
            resp = requests.get(url, timeout=timeout, headers={"User-Agent": "TechScan ML Trainer/1.0"})
    ...
    def generate_from_database(self, limit: int = 500) -> int:
        ...
        logger.info(
            f"Generated {count} training samples total (cache: {from_cache}, fetch: {from_fetch}, skipped: {skipped})"
        )
        return count
...
def train_from_database(limit: int = 500, save: bool = True) -> Dict[str, Any]:
    ...
    if count < 10:
        return {"error": f"Not enough samples: {count}", "min_required": 10}
...
def create_demo_training_data() -> Tuple[List[List[float]], List[List[str]]]:
    ...
    add_sample(
        {
            "html_length_bucket": 4,
            "script_count": 18,
            "link_count": 12,
            "pattern_wordpress": 45,
            "pattern_php": 20,
            "pattern_jquery": 12,
            "has_wordpress": True,
            "has_php": True,
    ...
    add_sample(
        ...
        ["WordPress", "PHP", "jQuery", "Elementor"],
        variations=10,
    )
    ...
    add_sample(
        ...
        ["WordPress", "Elementor", "Yoast SEO", "PHP"],
        variations=20,
    )
    ...
    add_sample(
        {
            ...
            "pattern_wordpress": 35,
            "has_wordpress": True,
            "pattern_elementor": 20,
            "has_elementor": True,
            "pattern_php": 15,
            "has_php": True,
            # Explicitly NO WooCommerce
            "pattern_woocommerce": 0,
