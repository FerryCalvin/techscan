...
class TechClassifier:
    ...
    def train(
        ...
        **kwargs,
        ...
        X_train, X_test, y_train, y_test = train_test_split(X_array, y_binary, test_size=test_size, random_state=42)
    ...
    def predict(self, html: str, headers: Dict[str, str], threshold: float = 0.3) -> List[Dict[str, Any]]:
        ...
        for i, tech in enumerate(TARGET_TECHNOLOGIES):
            ...
            if confidence >= threshold:
                predictions.append({"name": tech, "confidence": round(confidence * 100, 1), "source": "ml-classifier"})
    ...
    def predict_from_features(self, features: Dict[str, Any], threshold: float = 0.3) -> List[Dict[str, Any]]:
...
def predict_technologies(html: str, headers: Dict[str, str], threshold: float = 0.3) -> List[Dict[str, Any]]:
