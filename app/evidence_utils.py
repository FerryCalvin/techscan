...
def infer_snippet(url: str | None) -> str | None:
    ...
    if any(lowered.endswith(ext) for ext in (".woff2", ".woff", ".ttf", ".otf", ".eot")) or any(
        ext in lowered for ext in (".woff2?", ".woff?", ".ttf?", ".otf?", ".eot?")
    ):
...
def collect_header_evidence(
    tech_key: str, header_maps: Iterable[Tuple[str, dict[str, Any]]], aliases: Iterable[str] | None = None
) -> list[dict]:
    ...
    for source in sources:
        ...
        for item in source:
            normalised = (
                ...
                else normalize_evidence_entry(dict(item))
            )
            if normalised:
