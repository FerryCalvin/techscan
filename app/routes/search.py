...
def search_tech():
    ...
    try:
        ...
        logging.getLogger("techscan.search").error(
            "search error tech=%s category=%s version=%s err=%s", tech, category, version, e
        )
...
def diff_domain():
    ...
    try:
        ...
        with _db.get_conn() as conn:  # type: ignore[attr-defined]
            with conn.cursor() as cur:
                cur.execute(
                    """SELECT finished_at, technologies_json FROM scans
                               WHERE domain=%s ORDER BY finished_at DESC LIMIT 2""",
                    (domain_norm,),
                )
                rows = cur.fetchall()
        if len(rows) < 2:
            ...
            if latest_versions != prev_versions:
                ...
                if len(removed_versions) == 1 and len(added_versions) == 1:
                    ...
                    try:
                        ...
                        direction = (
                            "upgrade" if to_nums > from_nums else ("downgrade" if to_nums < from_nums else "change")
                        )
                    except Exception:
                        direction = "change"
...
def domain_suggest():
    ...
    try:
        with _db.get_conn() as conn:  # type: ignore[attr-defined]
            with conn.cursor() as cur:
                cur.execute(
                    """SELECT DISTINCT domain FROM scans WHERE LOWER(domain) LIKE %s ORDER BY domain ASC LIMIT %s""",
                    (prefix + "%", limit),
                )
                rows = cur.fetchall()
        suggestions = [r[0] for r in rows if r and r[0]]
