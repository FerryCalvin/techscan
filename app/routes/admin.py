...
def admin_auth():
    if not _check_auth():
        return jsonify(
            {"error": "unauthorized", "hint": "Set X-Admin-Token header or configure TECHSCAN_ADMIN_TOKEN"}
        ), 401
...
def cache_flush():
    ...
    logging.getLogger("techscan.admin").info(
        "cache flush domains=%s removed=%s remaining=%s",
        "subset" if domains else "all",
        res["removed"],
        res["remaining"],
    )
...
def weekly_rescan_run():
    ...
    try:
        summary = periodic.run_weekly_rescan_once(
            current_app, max_domains=limit, lookback_days=lookback, dry_run=dry_run
        )
...
def stats_view():
    ...
    try:
        ...
        stats["sniff_cache"] = {"entries": snap["entries"], "hits": snap["hits"], "misses": snap["misses"]}
...
def runtime_state():
    ...
    return jsonify(
        {"status": "ok", "flags": {"persist_browser": persist, "tiered": tiered, "version_audit": version_audit_flag}}
    )
...
def quick_diag():
    ...
    if (force_micro or not hres.get("technologies")) and os.environ.get("TECHSCAN_ULTRA_FALLBACK_MICRO", "0") == "1":
        ...
        try:
            ...
            micro = scan_utils.scan_domain(
                norm, current_app.config.get("WAPPALYZER_PATH"), timeout=micro_timeout, retries=0, full=False
            )
            ...
            if micro_techs:
                ...
                tier_meta["micro_engine_seconds"] = (
                    micro.get("timing", {}).get("engine_seconds") if isinstance(micro.get("timing"), dict) else None
                )
            else:
                tier_meta["micro_attempted"] = True
...
def runtime_update():
    ...
    logging.getLogger("techscan.admin").info(
        "runtime flags update previous=%s new=%s applied_keys=%s", previous, new_state, list(applied.keys())
    )
...
def version_dataset_update():
    ...
    path = (
        body.get("path")
        or os.environ.get("TECHSCAN_LATEST_VERSIONS_FILE")
        or str(pathlib.Path(__file__).resolve().parent.parent.parent / "data" / "latest_versions.json")
    )
    p = pathlib.Path(path)
    try:
    ...
    for budget in budgets:
        ...
        for _ in range(repeat):
            for dom in domains:
                try:
                    t0 = time.time()
                    res = scan_utils.quick_single_scan(
                        dom, wappalyzer_path=wpath, budget_ms=budget, defer_full=not disable_defer
                    )
