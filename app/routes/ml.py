...
def status():
    ...
    try:
        ...
        return jsonify(
            {"error": "scikit-learn not installed", "message": str(e), "install_command": "pip install scikit-learn"}
        ), 503
...
def predict():
    ...
    try:
        ...
        return jsonify({"error": "scikit-learn not installed", "install_command": "pip install scikit-learn"}), 503
    ...
    if url and not html:
        try:
            resp = requests.get(url, timeout=10, headers={"User-Agent": "TechScan ML/1.0"})
            html = resp.text
...
def extract_features():
    ...
    grouped = {
        "html_structure": {
            ...
            if k.startswith(
                ("html_", "has_doctype", "script_", "link_", "meta_", "div_", "img_", "form_", "iframe_", "js_", "css_")
            )
        },
        "headers": {
            k: v
            for k, v in features.items()
            if k.startswith(
                ("header_", "has_server", "has_x_", "has_content", "has_cache", "has_security", "server_", "powered_")
            )
        },
        ...
        "detected": {
            ...
            and not k.startswith(("has_doctype", "has_server", "has_x_", "has_content", "has_cache", "has_security"))
        },
        "meta": {
            k: v
            for k, v in features.items()
            if k.startswith("has_generator")
            or k.startswith("has_og")
            or k.startswith("has_twitter")
            or k.startswith("has_schema")
        },
    }

    return jsonify({"features": features, "grouped": grouped, "feature_count": len(features)})
...
def train():
    ...
    try:
        ...
        return jsonify({"error": "ML modules not available", "message": str(e)}), 503
...
def train_demo():
    ...
    try:
        metrics = _train_demo()
        return jsonify({"status": "success", "type": "demo", "metrics": metrics})
    except Exception as e:
...
def train_combined():
    ...
    try:
        ...
        return jsonify({"status": "success", "type": "combined", "metrics": metrics})
    except Exception as e:
...
def auto_learn_retrain():
    ...
    try:
        ...
        return jsonify({"status": "success", "type": "auto_learn_forced", "metrics": metrics})
    except ImportError:
        return jsonify({"error": "auto_learn module not available"}), 503
    except Exception as e:
...
def hybrid_scan():
    ...
    try:
        ...
        # Fetch HTML
        resp = requests.get(url, timeout=15, headers={"User-Agent": "TechScan Hybrid/1.0"})
        html = resp.text
        headers = dict(resp.headers)
