...
def version():
    uptime = time.time() - _START_TIME
    features = {"synthetic_headers": os.environ.get("TECHSCAN_SYNTHETIC_HEADERS", "1") == "1"}
    return jsonify(
        {"version": _VERSION, "git_commit": _COMMIT, "uptime_seconds": round(uptime, 2), "features": features}
    )
...
def api_system_health():
    ...
    out = {"enrichment": {"merge_total": merge_total}, "uptime_seconds": uptime}
