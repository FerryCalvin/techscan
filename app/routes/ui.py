...
_ICON_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "..", "node_modules", "tech-stack-icons", "icons")
)
                ...
                with conn.cursor() as cur:
                    ...
                    for cat_row in top_cats_raw:
                        ...
                        top_categories_with_techs.append(
                            {"category": cat_name, "rawCategory": cat_name, "count": cat_count, "techs": techs_in_cat}
                        )
                    ...
                    try:
                        ...
                        out["payload_stats"] = {"avg_24h": None, "avg_7d": None, "avg_30d": None}
        except Exception as e:
            _log.exception("api_stats db aggregation failed err=%s", e)
            out["db_error"] = str(e)
            out.setdefault("top_techs", [])
            out.setdefault("top_categories", [])
            out.setdefault("payload_stats", {"avg_24h": None, "avg_7d": None, "avg_30d": None})
    else:
...
def api_domain_delete(domain: str):
    ...
    try:
        ...
        pass
    _log.info(
        "api_domain_delete_ok domain=%s scans=%s tech_rows=%s groups=%s mem_cleared=%s",
        normalized,
        scans_deleted,
        tech_rows_deleted,
        groups_removed,
...
def api_domain_history(domain: str):
    ...
    return jsonify({"domain": domain, "limit": limit, "offset": offset, "total": total, "scans": rows})
        ...
        return jsonify(
            {
                ...
                "groups": obj.groups,
                "version": obj.version,
                "assigned": {"group": group, "domain": domain},
                "diag": diag,
            }
        )
    except ValueError as e:
...
def api_domain_groups_rename(group):
    try:
        ...
        return jsonify(
            {
                "status": "ok",
                "renamed": {"old": group, "new": new_name},
                "groups": obj.groups,
                "version": obj.version,
                "diag": diag,
            }
        ...
        return jsonify(
            {
                ...
                "groups": obj.groups,
                "version": obj.version,
                "removed": {"group": group, "domain": domain},
                "diag": diag,
            }
        )
    except ValueError as e:
    ...
    try:
        ...
        if include_hints and domains:
            try:
                ...
                logging.getLogger("techscan.ui").debug(
                    "failed fetching hint meta for tech=%s", tech_name, exc_info=True
                )
