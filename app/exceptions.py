...
class SSRFBlockedError(ValidationError):
    ...
    def __init__(self, domain: str):
        super().__init__(
            f"Domain appears to be internal/private: {domain}",
            details={'domain': domain, 'reason': 'SSRF protection'}
        )
...
class ScanFailedError(ScanError):
    ...
    def __init__(self, domain: str, reason: str):
        super().__init__(
            f"Scan failed for {domain}: {reason}",
...
def make_error_response(error_code: str, message: str, 
                        status_code: int = 500, 
