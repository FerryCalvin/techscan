...
class VersionExtractor:
    ...
    def extract_from_js_vars(self, js_vars: Dict[str, Any], tech_key: str) -> Optional[VersionResult]:
    ...
    def extract_from_urls(self, urls: List[str], tech_key: str) -> Optional[VersionResult]:
    ...
    def extract_from_meta(self, html: str, tech_key: str) -> Optional[VersionResult]:
    ...
    def extract_from_comments(self, html: str, tech_key: str) -> Optional[VersionResult]:
    ...
    def extract_version(
        ...
        if self._ml_verifier:
            is_valid, ml_confidence = self._ml_verifier.verify(
                best.tech_name, best.version, {"html": html[:1000], "urls": urls[:10]}
            )
            if not is_valid:
