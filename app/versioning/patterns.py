...
VERSION_PATTERNS: Dict[str, TechnologyVersionConfig] = {
    ...
    'wordpress': {
        'name': 'WordPress',
        'signals': [
            {'type': 'meta', 'pattern': r'<meta\s+name=["\']generator["\']\s+content=["\']WordPress\s+(\d+\.\d+(?:\.\d+)?)["\']', 'confidence': 1.0},
    ...
    'monsterinsights': {
        ...
        'signals': [
            {'type': 'url', 'pattern': r'/google-analytics-for-wordpress/[^"\']*\?ver=(\d+\.\d+\.\d+)', 'confidence': 0.95},
        ],
        'version_format': 'semver',
    },
}
