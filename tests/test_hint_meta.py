...
def test_attach_raw_hint_meta_merges_into_existing_bucket():
    payload = {
        ...
        'tiered': {
            'hint_meta': {
                'cookie_hits': [{'name': 'WordPress', 'label': 'wordpress_cookie'}]
            }
        }
    }
...
def test_hint_meta_detection_adds_jquery_migrate():
    payload = {
        ...
        'raw': {
            'extras': {
                'scripts': ['https://example.com/wp-includes/js/jquery/jquery-migrate.min.js?ver=3.4.0']
            }
        }
    }
    ...
    payload = {
        ...
        'raw': {
            'extras': {
                'scripts': [],
                'links': [],
                'body_classes': ['site', 'hello-elementor']
            }
        }
    }
    _apply_hint_meta_detections(payload)
...
def test_hint_meta_detection_detects_wpml_and_multisite():
    payload = {
        ...
        'raw': {
            'extras': {
                'scripts': ['https://example.com/wp-content/plugins/sitepress-multilingual-cms/res/js/sitepress.js?ver=4.6.3'],
