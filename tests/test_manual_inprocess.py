        ...
        rows.append(
            {
                ...
                "reason": (data.get("tiered") or {}).get("reason")
                or (data.get("tiered") or {}).get("heuristic_reason"),
...
def main():
    ...
    ap.add_argument(
        "--fast",
        action="store_true",
        help="Fast mode: reduce timeouts (baseline=20,tiered=20,persist=20 unless overridden).",
    )
