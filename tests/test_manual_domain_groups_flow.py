...
step(
    "assign_domain", lambda: client.post(f"/api/domain_groups/{NEW_GROUP}/assign", json={"domain": "example.com"}).json
)
step(
    "remove_domain", lambda: client.post(f"/api/domain_groups/{NEW_GROUP}/remove", json={"domain": "example.com"}).json
)
