...
try {
    (Invoke-RestMethod 'http://127.0.0.1:5000/admin/health' -TimeoutSec 10) | ConvertTo-Json -Depth 4
}
catch {
    Write-Host 'HEALTH ERROR'
    (Invoke-WebRequest 'http://127.0.0.1:5000/admin/health' -UseBasicParsing -TimeoutSec 10).Content
}
...
foreach ($d in $domains) {
    ...
    try {
        ...
        Write-Host 'SCAN ERROR'
        (Invoke-WebRequest "http://127.0.0.1:5000/scan?domain=$d&force=1&debug=1" -UseBasicParsing -TimeoutSec 120).Content
    }
}
